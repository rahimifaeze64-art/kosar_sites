...
class AgentChat {
    ...
    init() {
        if (this.initialized) {
            console.log('AgentChat already initialized, re-rendering messages');
            this.renderMessages();
            return;
        }
    ...
    loadMessages() {
        const saved = localStorage.getItem('agentChat_messages');
    ...
    loadParticipants() {
        ...
        const dynamicParticipants = users.filter(u => u.role === 'agent' || u.role === 'manager');
        this.participants = [...fixedAgents];
    ...
    setupEventListeners() {
        const sendBtn = document.getElementById('sendAgentChatBtn');
                ...
                <div class="space-y-2 mb-4">
                    ${this.participants.map(p => `
                        <div class="flex items-center gap-3 p-3 bg-slate-700 hover:bg-slate-600 rounded-lg cursor-pointer transition-all hover:translate-x-[-4px]" 
                             ...
                             onclick="document.getElementById('agentChatInput').value += '@${p.username} '; document.getElementById('agentChatInput').focus(); this.closest('.fixed').remove();">
                            ...
                            <span class="px-2 py-1 rounded-full text-xs font-semibold ${p.role === 'manager' ? 'bg-lime-600 text-white' : 'bg-amber-600 text-white'}">
    ...
    renderMessages() {
        ...
        if (!container) {
            console.warn('agentChatMessages container not found');
            return;
        }
                ...
                <div style="display: flex; justify-content: flex-start; margin-bottom: 1rem; animation: slideIn 0.3s ease; direction: rtl;" class="message-group">
                    <div style="max-width: 70%; display: flex; flex-direction: column; align-items: flex-start;">
                        <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem; font-size: 0.75rem;">
                            ...
                            <span style="padding: 0.125rem 0.5rem; border-radius: 9999px; font-size: 0.625rem; font-weight: 600; ${msg.senderRole === 'manager' ? 'background: #7c3aed; color: white;' : 'background: #f59e0b; color: white;'}">
                                ${this.getRoleName(msg.senderRole)}
                            </span>
                            <span style="color: #6b7280;">${time}</span>
                            <span style="color: #4b5563; font-size: 0.625rem;">${date}</span>
                        </div>
                        <div style="position: relative; border-radius: 0.75rem; padding: 0.75rem 1rem; ${isOwn ? 'background: #f59e0b; color: white;' : 'background: #475569; color: #f1f5f9;'}"
                             oncontextmenu="event.preventDefault(); if(window.agentChatInstance) window.agentChatInstance.showContextMenu(event, '${msg.id}'); return false;"
                             ontouchstart="if(window.agentChatInstance) window.agentChatInstance.handleLongPressStart(event, '${msg.id}')"
                             ontouchend="if(window.agentChatInstance) window.agentChatInstance.handleLongPressEnd()"
                             ontouchmove="if(window.agentChatInstance) window.agentChatInstance.handleLongPressEnd()">
        ...
        message.mentions.forEach(username => {
            ...
            if (user) {
                this.sendNotification(user.id, {
                    type: 'mention',
                    message: `${message.senderName} شما را در چت عامل‌ها منشن کرد`,
        ...
        menu.innerHTML = `
            <div class="py-2">
                ...
                ${message.type === 'file' || message.type === 'voice' ? `
                    <button onclick="window.agentChatInstance.saveToArchive('${messageId}')" 
                            class="w-full text-right px-4 py-2 hover:bg-slate-700 text-white flex items-center gap-3">
        ...
        modal.innerHTML = `
            <div class="bg-slate-800 rounded-2xl p-6 max-w-md w-11/12">
                ...
                <div class="flex gap-3 mt-4">
                    <button onclick="window.agentChatInstance.saveEditedMessage('${messageId}'); this.closest('.fixed').remove();" 
        ...
        modal.innerHTML = `
            <div class="bg-slate-800 rounded-2xl p-6 max-w-md w-11/12">
                ...
                <div class="flex gap-3">
                    <button onclick="window.agentChatInstance.confirmSaveToArchive('${messageId}'); this.closest('.fixed').remove();" 
            ...
            fetch(fileData)
                ...
                .then(blob => {
                    ...
                    navigator.share({
                        ...
                        text: `فایل از گفتگو عامل‌ها: ${fileName}`,
                        files: [file]
                    })
...
// راه‌اندازی
let agentChatInstance;

document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('agentChatMessages') && !window.agentChatInstance) {
        window.agentChatInstance = new AgentChat();
        console.log('AgentChat initialized on DOMContentLoaded');
    }
});
