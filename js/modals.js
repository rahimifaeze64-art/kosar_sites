...
const ModalsModule = {
    ...
    getAlpineData() {
        try {
            ...
            if (dataElement && dataElement._x_dataStack && dataElement._x_dataStack[0]) {
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select x-model="newOrder.type" class="form-control" required 
                            ...
                            <option value="">انتخاب کنید</option>
                            ${Object.values(CONFIG.ORDER_TYPES).map(type => `<option value="${type}">${type}</option>`).join('')}
                        </select>
                    </div>
    ...
    getCreateProjectModalOld() {
        ...
        const doctors = DataModule.getUsers().filter(u => u.role === CONFIG.ROLES.DOCTOR);
        const students = DataModule.getUsers().filter(u => u.role === CONFIG.ROLES.STUDENT);
    ...
    async submitNewOrder(orderData) {
        try {
            ...
            if (!orderData || !orderData.studentName || !orderData.nickname || 
                !orderData.birthDate || !orderData.passportNumber || !orderData.gender ||
                !orderData.university || !orderData.field || !orderData.degree || 
                !orderData.phone || !orderData.behestanUsername || !orderData.behestanPassword ||
                !orderData.studentId || !orderData.deadline) {
                debugLogger('Validation failed - missing required fields', 'error', orderData);
            ...
            try {
                if (typeof APIDataModule !== 'undefined') {
                    ...
                    const studentData = {
                        user: {
                            first_name: orderData.studentName.split(' ')[0] || orderData.studentName,
                            last_name: orderData.studentName.split(' ').slice(1).join(' ') || '',
                    ...
                    debugLogger('Creating student profile via API...', 'info', studentData);
                    ...
                    debugLogger('Order created successfully via API', 'success', newOrder);
                    UTILS.showNotification('سفارش با موفقیت در سیستم ثبت شد', 'success');
                }
            } catch (apiError) {
                debugLogger('API order creation failed, using localStorage fallback', 'warning', apiError);
                useAPI = false;
            }
            ...
            if (!useAPI) {
                ...
                newOrder = {
                    ...
                    type: orderData.workList && orderData.workList.length > 0 ? orderData.workList.join(', ') : 'سفارش چندگانه',
                    deadline: orderData.deadline,
                    description: orderData.description || '',
                ...
                debugLogger('Order saved to localStorage', 'success', { orderId: newOrder.id });
                ...
                if (assignedId && window.TasksModule && typeof TasksModule.createTaskFromOrder === 'function') {
                ...
                if (orderData.workAssignments && typeof orderData.workAssignments === 'object') {
                    const users = DataModule.getUsers();
                    const assignedAgentIds = [...new Set(Object.values(orderData.workAssignments).filter(Boolean))];
                    assignedAgentIds.forEach(agentId => {
                        if (agentId && agentId !== assignedId) {
                            ...
                            if (agent && window.TasksModule && typeof TasksModule.createTaskFromOrder === 'function') {
                                TasksModule.createTaskFromOrder(newOrder, agent);
                                debugLogger(`Task created for agent ${agent.name} (multi-assign)`, 'success');
                            }
                        }
                    });
                }
            ...
            debugLogger('Error submitting new order', 'error', { message: error.message, stack: error.stack });
    ...
    submitNewUser(userData) {
        // Validation
        if (!userData.role || !userData.name || !userData.username || !userData.password) {
            UTILS.showNotification('لطفاً تمام فیلدهای الزامی را پر کنید', 'error');
            return;
        }
        ...
        if (userData.role === 'student') {
            if (!userData.university || !userData.studentId || !userData.field || !userData.degree) {
                UTILS.showNotification('لطفاً تمام فیلدهای الزامی دانشجو را پر کنید', 'error');
                return;
            }
        }
        ...
        if (userData.role === 'translator' && userData.languages) {
            userData.languages = userData.languages.split(',').map(lang => lang.trim());
        }
    ...
    submitEditUser(userData) {
        ...
        // Process languages for translator
        if (userData.role === 'translator' && typeof userData.languages === 'string') {
            userData.languages = userData.languages.split(',').map(lang => lang.trim());
        }
    ...
    submitCreateProject(projectData) {
        try {
            ...
            // Validation
            if (!projectData || !projectData.studentId || !projectData.assignedDoctorId || 
                !projectData.type || !projectData.degree || !projectData.deadline) {
                debugLogger('Validation failed - missing required fields', 'error', projectData);
                UTILS.showNotification('لطفاً تمام فیلدهای الزامی را پر کنید', 'error');
                return;
            }
            ...
            try {
                totalAmount = UTILS.calculateOrderPrice(projectData.type, projectData.degree) || 0;
                shares = UTILS.calculateShares(projectData.type, totalAmount) || { doctorShare: 0, managerShare: 0 };
            } catch (e) {
            ...
            debugLogger('Project pricing calculated', 'info', { totalAmount, shares });
            ...
            debugLogger('New project saved successfully', 'success', { projectId: newProject.id });
            UTILS.showNotification(`پروژه با موفقیت ایجاد و به ${projectData.assignedDoctor} تخصیص یافت`, 'success');
            ...
            debugLogger('Error creating project', 'error', { message: error.message, stack: error.stack });
    ...
    submitStudentProject(projectData) {
        try {
            ...
            // Validation
            if (!projectData || !projectData.type || !projectData.degree || !projectData.deadline) {
                debugLogger('Validation failed - missing required fields', 'error', projectData);
                UTILS.showNotification('لطفاً تمام فیلدهای الزامی را پر کنید', 'error');
                return;
            }
            ...
            debugLogger('Student project pricing calculated', 'info', { totalAmount, shares });
            ...
            debugLogger('Student project request saved successfully', 'success', { projectId: newProject.id });
            UTILS.showNotification('درخواست پروژه با موفقیت ثبت شد و برای بررسی ارسال گردید', 'success');
            ...
            debugLogger('Error creating student project request', 'error', { message: error.message, stack: error.stack });
