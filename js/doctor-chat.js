...
class DoctorChat {
    ...
    init() {
        if (this.initialized) {
            console.log('DoctorChat already initialized, re-rendering messages');
            this.renderMessages();
            return;
        }
        ...
        console.log('DoctorChat initialized successfully');
    }

    loadMessages() {
        const saved = localStorage.getItem('doctorChat_messages');
        this.messages = saved ? JSON.parse(saved) : [];
    }

    saveMessages() {
        localStorage.setItem('doctorChat_messages', JSON.stringify(this.messages));
    }
    ...
    loadParticipants() {
        ...
        this.participants = [...fixedDoctors];
    ...
    setupEventListeners() {
        const sendBtn = document.getElementById('sendDoctorChatBtn');
        const messageInput = document.getElementById('doctorChatInput');
                ...
                <div class="space-y-2 mb-4">
                    ${this.participants.map(p => `
                        <div class="flex items-center gap-3 p-3 bg-slate-700 hover:bg-slate-600 rounded-lg cursor-pointer transition-all hover:translate-x-[-4px]" 
                             ...
                             onclick="document.getElementById('doctorChatInput').value += '@${p.username} '; document.getElementById('doctorChatInput').focus(); this.closest('.fixed').remove();">
                            ...
                            <span class="px-2 py-1 rounded-full text-xs font-semibold ${p.role === 'manager' ? 'bg-purple-600 text-white' : 'bg-emerald-600 text-white'}">
    ...
    renderMessages() {
        ...
        if (!container) {
            console.warn('doctorChatMessages container not found');
            return;
        }
                ...
                <div style="display: flex; justify-content: flex-start; margin-bottom: 1rem; animation: slideIn 0.3s ease; direction: rtl;" class="message-group">
                    <div style="max-width: 70%; display: flex; flex-direction: column; align-items: flex-start;">
                        <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem; font-size: 0.75rem;">
                            ...
                            <span style="padding: 0.125rem 0.5rem; border-radius: 9999px; font-size: 0.625rem; font-weight: 600; ${msg.senderRole === 'manager' ? 'background: #7c3aed; color: white;' : 'background: #10b981; color: white;'}">
                                ${this.getRoleName(msg.senderRole)}
                            </span>
                            <span style="color: #6b7280;">${time}</span>
                            <span style="color: #4b5563; font-size: 0.625rem;">${date}</span>
                        </div>
                        <div style="position: relative; border-radius: 0.75rem; padding: 0.75rem 1rem; ${isOwn ? 'background: #10b981; color: white;' : 'background: #475569; color: #f1f5f9;'}"
                             oncontextmenu="event.preventDefault(); if(window.doctorChatInstance) window.doctorChatInstance.showContextMenu(event, '${msg.id}'); return false;"
                             ontouchstart="if(window.doctorChatInstance) window.doctorChatInstance.handleLongPressStart(event, '${msg.id}')"
                             ontouchend="if(window.doctorChatInstance) window.doctorChatInstance.handleLongPressEnd()"
                             ontouchmove="if(window.doctorChatInstance) window.doctorChatInstance.handleLongPressEnd()">
        ...
        message.mentions.forEach(username => {
            ...
            if (user) {
                this.sendNotification(user.id, {
                    type: 'mention',
                    message: `${message.senderName} شما را در چت عاملها منشن کرد`,
        ...
        menu.innerHTML = `
            <div class="py-2">
                ...
                ${message.type === 'file' || message.type === 'voice' ? `
                    <button onclick="window.doctorChatInstance.saveToArchive('${messageId}')" 
                            class="w-full text-right px-4 py-2 hover:bg-slate-700 text-white flex items-center gap-3">
        ...
        modal.innerHTML = `
            <div class="bg-slate-800 rounded-2xl p-6 max-w-md w-11/12">
                ...
                <div class="flex gap-3 mt-4">
                    <button onclick="window.doctorChatInstance.saveEditedMessage('${messageId}'); this.closest('.fixed').remove();" 
        ...
        modal.innerHTML = `
            <div class="bg-slate-800 rounded-2xl p-6 max-w-md w-11/12">
                ...
                <div class="flex gap-3">
                    <button onclick="window.doctorChatInstance.confirmSaveToArchive('${messageId}'); this.closest('.fixed').remove();" 
    ...
    forwardFile(messageId) {
        ...
        modal.innerHTML = `
            <div class="bg-slate-800 rounded-2xl p-6 max-w-md w-11/12">
                <h3 class="text-xl font-bold text-white mb-4">
                    <i class="fas fa-share text-purple-400 ml-2"></i>
                    فوروارد به
                </h3>
                <div class="space-y-2 mb-4">
                    <button onclick="window.doctorChatInstance.forwardToArchive('${messageId}'); this.closest('.fixed').remove();" 
            ...
            fetch(fileData)
                ...
                .then(blob => {
                    ...
                    navigator.share({
                        ...
                        text: `فایل از گفتگو عاملها: ${fileName}`,
                        files: [file]
                    })
...
// راه‌اندازی
let doctorChatInstance;

document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('doctorChatMessages') && !window.doctorChatInstance) {
        window.doctorChatInstance = new DoctorChat();
        console.log('DoctorChat initialized on DOMContentLoaded');
    }
});
