...
function appController() {
    return {
        ...
        dashboardContent: '<div class="text-center text-gray-500">در حال بارگذاری...</div>',
        ordersContent: '',
            ...
            this.subscribeToRealtimeEvents();
                ...
                if (savedUser) {
                    try {
                        ...
                        debugLogger('Current user loaded from localStorage', 'success', this.currentUser);
                    } catch (e) {
                ...
                if (!enableAPI) {
                    ...
                    try {
                        ...
                        debugLogger('API initialization failed, using localStorage mode', 'warning', error);
                        this.apiMode = false;
        ...
        async switchUser() {
            try {
                ...
                // لیست ثابت کارمند‌ها
                const employees = [
                    { id: 'emp001', name: 'ساره', username: 'zahra', role: 'employee', email: 'zahra@edu-system.com' },
                    { id: 'emp002', name: 'زینب', username: 'fatemeh', role: 'employee', email: 'fatemeh@edu-system.com' },
                    { id: 'emp003', name: 'فرزاد', username: 'farzad', role: 'employee', email: 'farzad@edu-system.com' },
                    { id: 'emp004', name: 'سلیمان', username: 'soleiman', role: 'employee', email: 'soleiman@edu-system.com' }
                ];
                
                // اول چک کن آیا کارمند است
                if (this.selectedUserId.startsWith('emp')) {
                    const selectedUser = employees.find(c => c.id === this.selectedUserId);
                    if (selectedUser) {
                        this.currentUser = { ...selectedUser };
                        // Save to localStorage
                        localStorage.setItem('currentUser', JSON.stringify(this.currentUser));
                        debugLogger('employee switched successfully', 'success', this.currentUser);
                        UTILS.showNotification(`وارد شدید به عنوان: ${selectedUser.name}`, 'success');
                ...
                if (selectedUser) {
                    ...
                    // Save to localStorage
                    localStorage.setItem('currentUser', JSON.stringify(this.currentUser));
                    debugLogger('User switched successfully', 'success', this.currentUser);
                    UTILS.showNotification(`وارد شدید به عنوان: ${selectedUser.name}`, 'success');
                    ...
                    if (selectedUser.role === 'employee') {
                        ...
                        this.currentPage = 'myAgentTasks';
                        debugLogger('Setting page to myAgentTasks for agent', 'info', { 
        ...
        async loadDashboardContent() {
            try {
                debugLogger('Loading dashboard content...', 'info', { role: this.currentUser.role });
                if (!DashboardModule) {
                    debugLogger('DashboardModule not found', 'error');
                    this.dashboardContent = '<div class="text-red-500">خطا: ماژول داشبورد یافت نشد</div>';
                    return;
                }
                
                // Check if we have the enhanced async version
                if (typeof DashboardModule.getDashboardContent === 'function') {
                    this.dashboardContent = await DashboardModule.getDashboardContent(this.currentUser.role);
                    debugLogger('Dashboard content loaded', 'success');
                } else {
                    debugLogger('Using fallback dashboard', 'warning');
                    this.dashboardContent = '<div class="text-yellow-500">داشبورد در حال بارگذاری...</div>';
                }
            } catch (error) {
                debugLogger('Error loading dashboard content', 'error', error);
                this.dashboardContent = '<div class="text-red-500">خطا در بارگذاری داشبورد</div>';
            }
        },
        ...
        async getOrdersContent() {
            ...
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                try {
                    debugLogger(`Loading orders content (attempt ${attempt}/${maxAttempts})...`, 'info', { 
                    ...
                    // Get content
                    const content = await OrdersMod.getOrdersContent(this.currentUser.role, this.currentUser.id);
                    debugLogger('Orders content loaded successfully', 'success');
                    return content;
                    
                } catch (error) {
                    console.error(`❌ Attempt ${attempt} failed:`, error);
                    debugLogger(`Error loading orders content (attempt ${attempt})`, 'error', {
                        name: error.name,
        ...
        async loadOrdersPage() {
            this.ordersContent = '<div class="text-center py-8"><i class="fas fa-spinner fa-spin text-4xl text-blue-500"></i><p class="mt-4 text-white">در حال بارگذاری...</p></div>';
        ...
        async loadOrdersPageWithRetry(maxRetries = 5) {
            ...
            console.log('  - OrdersModule exists?', typeof window.OrdersModule !== 'undefined');
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    if (attempt === maxRetries) {
                        this.ordersContent = '<div class="text-center text-red-500 py-8"><i class="fas fa-exclamation-triangle text-4xl mb-4"></i><p>خطا در بارگذاری سفارشات</p><button onclick="location.reload()" class="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">تلاش مجدد</button></div>';
                        return;
                    }
            ...
            console.error('❌ Failed to load OrdersModule after', maxRetries, 'attempts');
            this.ordersContent = '<div class="text-center text-red-500 py-8"><i class="fas fa-exclamation-triangle text-4xl mb-4"></i><p>خطا در بارگذاری سفارشات</p><p class="text-sm mt-2">OrdersModule یافت نشد</p><button onclick="location.reload()" class="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">تلاش مجدد</button></div>';
        },
        ...
        getUsersContent() {
            try {
                debugLogger('Loading users content...', 'info', { role: this.currentUser.role });
        ...
        getTasksContent() {
            try {
                debugLogger('Loading tasks content...', 'info', { role: this.currentUser.role });
                // TEST MODE: Temporarily disabled access check
                // if (this.currentUser.role !== CONFIG.ROLES.MANAGER) {
                //     debugLogger('Access denied: not manager', 'warning');
                //     return '<div class="text-yellow-500">دسترسی محدود: فقط مدیر</div>';
        ...
        getMyTasksContent() {
            try {
                debugLogger('Loading my tasks content...', 'info', { role: this.currentUser.role });
                // TEST MODE: Temporarily disabled access check
        ...
        getChatWithManagerContent() {
            try {
                debugLogger('Loading chat with manager content...', 'info', { role: this.currentUser.role });
                // TEST MODE: Temporarily disabled access check
                // if (this.currentUser.role !== CONFIG.ROLES.employee) {
                //     debugLogger('Access denied: not employee', 'warning');
                //     return '<div class="text-yellow-500">دسترسی محدود: فقط کارمند</div>';
                ...
                const content = EmployeeModule.getChatWithManagerContent(this.currentUser.id);
        ...
        getManagementChatContent() {
            try {
                debugLogger('Loading management chat content...', 'info', { role: this.currentUser.role });
                // TEST MODE: Temporarily disabled access check
                ...
                const content = ManagementChatModule.getManagementChatContent(this.currentUser);
        ...
        getWorkHoursContent() {
            try {
                debugLogger('Loading work hours content...', 'info', { role: this.currentUser.role });
        ...
        getAgentChatContent() {
            try {
                debugLogger('Loading agent chat content...', 'info', { role: this.currentUser.role });
                // TEST MODE: Temporarily disabled access check
        ...
        showNotifications() {
            try {
                ...
                this.notifications = [
                    { id: 1, message: 'سفارش جدید ثبت شد', type: 'info', time: '5 دقیقه پیش' },
                    { id: 2, message: 'رساله تایید شد', type: 'success', time: '1 ساعت پیش' }
                ];
        ...
        viewOrder(orderId) {
            try {
                ...
                if (this.selectedOrder) {
                    ...
                    debugLogger('Order found and modal opened', 'success', this.selectedOrder);
                } else {
                    debugLogger('Order not found', 'warning', { orderId, availableOrders: orders.map(o => o.id) });
                }
            } catch (error) {
            ...
            try {
                if ('serviceWorker' in navigator) {
                    ...
                    navigator.serviceWorker.register('/sw.js')
                        .then(registration => {
                            debugLogger('Service worker registered successfully', 'success', registration);
                        })
                        .catch(registrationError => {
                            debugLogger('Service worker registration failed', 'error', registrationError);
                        });
                } else {
...
window.submitAssignOrder = function(doctorId) {
    try {
        ...
        orders[orderIndex].stage = `عامل در حال نوشتن ${orders[orderIndex].type === 'نوشتن رساله' ? 'رساله' : orders[orderIndex].type === 'نوشتن مقاله' ? 'مقاله' : 'سفارش'} شما است`;
        ...
        debugLogger('Order assigned successfully', 'success', { orderId, doctorId });
...
window.approveOrder = function(orderId) {
    try {
        ...
        orders[orderIndex].stage = 'مدیر پروژه را تایید کرد - هماهنگی در حال انجام است';
...
window.submitRejectOrder = function(reason) {
    try {
        ...
        // Get current user
        const currentUser = ModalsModule ? ModalsModule.getCurrentUser() : { id: 'mgr001', name: 'مدیر', role: 'manager' };
...
window.showAssignmentModal = window.showAssignmentModal || function(orderId) {
    debugLogger('showAssignmentModal fallback called', 'warning', { orderId });
    if (typeof WorkflowModule !== 'undefined' && WorkflowModule.showAssignmentModal) {
        WorkflowModule.showAssignmentModal(orderId);
    } else {
        UTILS.showNotification('سیستم تخصیص در حال بارگذاری است، لطفاً کمی صبر کنید', 'warning');
...
window.viewOrderDetails = window.viewOrderDetails || function(orderId) {
    debugLogger('viewOrderDetails fallback called', 'warning', { orderId });
    if (typeof WorkflowModule !== 'undefined' && WorkflowModule.viewOrderDetails) {
        WorkflowModule.viewOrderDetails(orderId);
    } else {
        UTILS.showNotification('سیستم مشاهده جزئیات در حال بارگذاری است', 'warning');
        setTimeout(() => {
            if (typeof WorkflowModule !== 'undefined' && WorkflowModule.viewOrderDetails) {
                WorkflowModule.viewOrderDetails(orderId);
            } else {
                UTILS.showNotification('خطا در بارگذاری سیستم مشاهده جزئیات', 'error');
            }
        }, 1000);
    }
};
...
window.openAddUserModal = function() {
    ...
    try {
        ...
        if (appElement && appElement.__x && appElement.__x.$data) {
            ...
            window.dispatchEvent(new CustomEvent('openModal', { detail: { modal: 'addUser' } }));
        }
    } catch (error) {
...
window.downloadProfile = function() {
    try {
        const currentUser = ModalsModule ? ModalsModule.getCurrentUser() : { id: 'mgr001', name: 'مدیر' };
...
window.agentToggleTaskStatus = function(taskId, userId) {
    ...
    // بازخوانی صفحه
    const container = document.querySelector('[x-show="currentPage === \'myAgentTasks\'"]');
    if (container && typeof window.getMyAgentTasksContent === 'function') {
...
window.getMyAgentTasksContent = function() {
    try {
        ...
        const inProgressTasksCount = myTasks.filter(t => t.status === 'in_progress').length;
                ...
                <div class="bg-slate-800 rounded-lg p-4">
                    ...
                    ` : `
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ${myTasks.filter(t => t.isOrderTask).map(task => window.getAgentTaskCard(task, currentUser.id)).join('')}
                        </div>
                    `}
                </div>
        ...
        return `
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-${Object.keys(incomeByCurrency).length || 1} gap-4">
                    ...
                    ` : Object.entries(incomeByCurrency).map(([currency, total]) => `
                ...
                <div class="bg-slate-800 rounded-lg shadow-md p-4">
                    ...
                    ` : `
                        <div class="space-y-3">
                            ${Object.entries(incomeByWork).map(([work, data]) => `
