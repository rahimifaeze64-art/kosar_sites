...
const EmployeeModule = {
    ...
    getMyTasksContent(userId) {
        ...
        return `
            <div class="space-y-6">
                ...
                ════════════════════════════════════════════ -->
                <div class="bg-slate-800 rounded-lg shadow-md p-5 border-r-4 border-lime-500">
                    <div class="flex items-center justify-between mb-1">
                        <h3 class="text-lg font-bold text-white flex items-center gap-2">
                            <i class="fas fa-graduation-cap text-lime-400"></i>
                            کارتابل — مراحل دانشجویان
                            ${pendingStepTasks.length > 0
                                ? `<span class="bg-lime-600 text-gray-900 text-xs rounded-full px-2 py-0.5 animate-pulse">${pendingStepTasks.length}</span>`
    ...
    getStepTaskCard(task, userId, num) {
        ...
        const color = typeColors[task.stepType] || 'lime';
        const typeName = typeNames[task.stepType] || task.stepType || '';

        const statusMap = {
            pending:     { label: 'در انتظار',    cls: 'bg-lime-500' },
                ...
                <div class="flex items-center space-x-3 space-x-reverse">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="w-full bg-slate-500 rounded-full h-1">
                            <div class="bg-lime-400 h-1 rounded-full" style="width: 0%" id="emp-progress-${task.id}"></div>
                        </div>
                    </div>
                ...
                <div class="flex justify-between items-start">
                    <div class="flex-1">
                        <div class="flex items-center mb-2">
                            <span class="bg-lime-600 text-gray-900 text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center ml-2">${taskNumber}</span>
                ...
                <div class="flex flex-wrap gap-3 items-start justify-between">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <button onclick="employeeModule.samLoginForStudent();"
                            ...
                            <span class="hidden sm:inline">ورود به سامانه</span>
                        </button>
    ...
    _renderStudentTable(students, emptyMsg) {
        ...
        const _buildRow = (s, idx) => {
            ...
            const degreeLabels = { bachelor: 'کارشناسی', masters: 'کارشناسی ارشد', phd: 'دکتری', 'کارشناسی': 'کارشناسی', 'کارشناسی ارشد': 'کارشناسی ارشد', 'ارشد': 'کارشناسی ارشد', 'دکتری': 'دکتری', 'دكتراه': 'دکتری' };
            const degreeLabel = degreeLabels[s.degree] || s.degree || '—';
            ...
            return { s, rowNumber, degreeLabel, pct, curStep, barColor };
        };
        // ── کارت موبایل (< md) ────────────────────────────────
        const mobileCards = students.map((s, idx) => {
            const { rowNumber, degreeLabel, pct, curStep, barColor } = _buildRow(s, idx);
            return `
            <div class="bg-slate-700 rounded-xl p-4 space-y-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-xs text-gray-400">شماره دانشجویی: ${s.studentId || '—'}</div>
                            <div class="text-xs text-gray-400">رمز سامانه: ${s.systemPassword || '—'}</div>
                        </div>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-gray-400">
                    ...
                    <div class="truncate"><i class="fas fa-graduation-cap ml-1 text-lime-400"></i>${degreeLabel}</div>
                ...
                <div class="flex gap-2">
                    <button onclick="employeeModule.openStudentFlowchart('${s.id}')"
                        class="flex-1 bg-lime-600 hover:bg-lime-700 text-gray-900 py-2 rounded-lg text-xs font-semibold transition-all">
                        <i class="fas fa-project-diagram ml-1"></i>فلوچارت
                    </button>
                    <button onclick="employeeModule.samLoginForStudent()"
                        class="flex-1 bg-lime-600 hover:bg-lime-700 text-gray-900 py-2 rounded-lg text-xs font-semibold transition-all">
                        <i class="fas fa-robot ml-1"></i>ورود به سامانه
                    </button>
                </div>
        ...
        const tableRows = students.map((s, idx) => {
            ...
            <tr class="border-b border-slate-700 hover:bg-slate-700/50 transition-colors">
                <td class="px-3 py-3 text-sm font-bold text-gray-300 text-center">${rowNumber}</td>
                <td class="px-4 py-3">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-full bg-lime-600 flex items-center justify-center text-gray-900 font-bold text-[10px] flex-shrink-0" title="${s.name || ''}">
                            ${s.studentId || '—'}
                        </div>
                        <div class="font-semibold text-white text-sm">${s.name || '—'}</div>
                    </div>
                </td>
                <td class="px-4 py-3 text-sm text-gray-300" dir="ltr">${s.studentId || '—'}</td>
                <td class="px-4 py-3 text-sm text-gray-300" dir="ltr">${s.systemPassword || '—'}</td>
                <td class="px-4 py-3 text-sm text-gray-300">${degreeLabel}</td>
                ...
                <td class="px-4 py-3 text-center">
                    <button onclick="employeeModule.openStudentFlowchart('${s.id}')"
                        class="bg-lime-600 hover:bg-lime-700 text-gray-900 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap">
                        <i class="fas fa-project-diagram ml-1"></i>فلوچارت
                    </button>
                </td>
                <td class="px-4 py-3 text-center">
                    <div class="flex items-center justify-center gap-1.5">
                        <button onclick="employeeModule.samLoginForStudent()"
                            class="bg-lime-600 hover:bg-lime-700 text-gray-900 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap"
                            title="ورود مستقیم به سامانه دانشگاه">
                            <i class="fas fa-robot ml-1"></i>ورود به سامانه
                        </button>
                        <button onclick="employeeModule.editStudentProfile('${s.id}')"
            ...
            <table class="w-full text-right">
                <thead>
                    <tr class="bg-slate-900/60 text-gray-400 text-xs">
                        ...
                        <th class="px-4 py-3 text-right font-semibold">رمز سامانه</th>
                        <th class="px-4 py-3 text-right font-semibold">مقطع</th>
                        <th class="px-4 py-3 text-right font-semibold">رشته</th>
                        <th class="px-4 py-3 text-right font-semibold">پیشرفت</th>
                        <th class="px-4 py-3 text-center font-semibold">فلوچارت</th>
    ...
    // ── باز کردن فلوچارت مسیر تحصیلی یک دانشجوی مشخص ─────────
            ...
            <div class="bg-slate-700 rounded-lg p-4 hover:bg-slate-600 transition-all">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center">
                        <div class="w-12 h-12 rounded-full bg-lime-500 flex items-center justify-center text-gray-900 font-bold text-lg ml-3">
                ...
                <div class="space-y-2 text-sm text-gray-300 mb-4">
                    <div class="flex items-center">
                        <i class="fas fa-university text-lime-400 w-5 ml-2"></i>
                        <span>${student.university || 'نامشخص'}</span>
                    </div>
                    <div class="flex items-center">
                        <i class="fas fa-graduation-cap text-lime-400 w-5 ml-2"></i>
                        <span>${student.degree || 'نامشخص'} - ${student.field || 'نامشخص'}</span>
                    </div>
                    <div class="flex items-center">
                        <i class="fas fa-phone text-lime-400 w-5 ml-2"></i>
                        <span>${student.phone || 'نامشخص'}</span>
                    </div>
                </div>
                
                <button onclick="employeeModule.editStudentProfile('${student.id}')" 
                        class="w-full bg-lime-600 hover:bg-lime-700 text-gray-900 px-4 py-2 rounded-lg text-sm font-medium transition-all">
    ...
    switchStudentTab(tabName) {
        ...
        ['info', 'defense', 'graduation'].forEach(tab => {
            ...
            if (tab === tabName) {
                ...
                tabBtn.classList.add('border-lime-500', 'text-lime-400');
                content.style.display = 'block';
            } else {
                tabBtn.classList.remove('border-lime-500', 'text-lime-400');
                    ...
                    <div class="flex items-center space-x-3 space-x-reverse group bg-gray-50 rounded-lg p-3 hover:bg-gray-100 transition-all">
                        ...
                        <div class="flex-shrink-0 flex space-x-2 space-x-reverse opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onclick="employeeModule.moveEducationalStepUp('${student.id}', ${index})" 
                                    class="text-lime-600 hover:text-lime-700 p-2 ${index === 0 ? 'opacity-30 cursor-not-allowed' : ''}" 
                                    title="انتقال به بالا" ${index === 0 ? 'disabled' : ''}>
                                <i class="fas fa-arrow-up"></i>
                            </button>
                            <button onclick="employeeModule.moveEducationalStepDown('${student.id}', ${index})" 
                                    class="text-lime-600 hover:text-lime-700 p-2 ${index === steps.length - 1 ? 'opacity-30 cursor-not-allowed' : ''}" 
                        ...
                        <div class="flex-shrink-0 flex space-x-2 space-x-reverse opacity-0 group-hover:opacity-100 transition-opacity">
                            ${step.name === 'تحدید مناقشه' ? `
                                <button onclick="employeeModule.editDefenseStepDetails('${student.id}', ${index})" 
                                        class="text-lime-600 hover:text-lime-700 p-2" title="ویرایش جزئیات">
        ...
        return `
            ...
            <div class="bg-slate-700 rounded-lg p-4">
                <h4 class="font-bold text-white mb-3">
                    <i class="fas fa-user text-lime-400 ml-2"></i>
                    اطلاعات شخصی
                </h4>
            ...
            <div class="bg-slate-700 rounded-lg p-4">
                <h4 class="font-bold text-white mb-3">
                    <i class="fas fa-graduation-cap text-lime-400 ml-2"></i>
                    اطلاعات تحصیلی
                </h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
            ...
            <div class="bg-slate-700 rounded-lg p-6">
                ...
                <div class="mb-8">
                    <div class="w-full bg-slate-600 rounded-full h-3">
                        <div class="bg-gradient-to-r from-blue-500 to-lime-500 h-3 rounded-full transition-all duration-500" 
            ...
            <div class="flex items-center space-x-4 space-x-reverse p-4 bg-slate-600 rounded-lg hover:bg-slate-500 transition-all">
                ...
                <div class="flex-shrink-0">
                    ${isCompleted 
                        ...
                        : '<span class="text-xs px-3 py-1 bg-lime-500 text-gray-900 rounded-full">در انتظار</span>'
                    }
                </div>
            </div>
        `;
            ...
            <div id="step-details-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-slate-800 rounded-lg max-w-lg w-full">
                    <div class="p-6 border-b border-slate-700">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-bold text-white">
                                <i class="fas fa-sticky-note text-lime-400 ml-2"></i>
                                ${step.name}
                            </h3>
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        ${step.date ? `
                            <div class="flex items-center justify-between bg-slate-700 rounded-lg p-4">
                                <span class="text-gray-300">تاریخ تکمیل:</span>
                                <span class="text-white font-bold">
                                    <i class="fas fa-calendar ml-2 text-lime-400"></i>
                                    ${step.date}
                                </span>
                            </div>
                        ` : ''}
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                <i class="fas fa-edit text-lime-400 ml-1"></i>
                                یادداشت (اختیاری)
                            </label>
                            <textarea id="step-notes-input" 
                                      rows="5"
                                      placeholder="توضیحات، یادداشت‌ها یا نکات مربوط به این مرحله را وارد کنید..."
                                      class="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-3 text-white resize-none focus:border-lime-500 focus:ring-1 focus:ring-lime-500"
                            >${step.notes || ''}</textarea>
                            <p class="text-xs text-gray-500 mt-2">
                                <i class="fas fa-info-circle ml-1"></i>
                                این یادداشت فقط برای شما قابل مشاهده است
                    ...
                    <div class="p-6 border-t border-slate-700 flex justify-end space-x-3 space-x-reverse">
                        ...
                        <button onclick="employeeModule.saveStepNotes('${studentId}', ${stepIndex})" 
                                class="bg-lime-600 hover:bg-lime-700 text-gray-900 px-6 py-2 rounded-lg font-medium">
                            <i class="fas fa-save ml-2"></i>
                            ذخیره یادداشت
                        </button>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="bg-slate-800 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            ...
                            <i class="fas fa-clipboard-list text-3xl text-lime-400"></i>
                        </div>
                    </div>
                    <div class="bg-slate-800 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-400 text-sm">در انتظار</p>
                                <p class="text-2xl font-bold text-lime-400">${pendingTasks.length}</p>
                            </div>
                            <i class="fas fa-clock text-3xl text-lime-400"></i>
                        </div>
                    </div>
                ...
                <div class="bg-slate-800 rounded-lg p-2">
                    <div class="flex space-x-2 space-x-reverse">
                        <button onclick="employeeModule.filterAgentTasks('all', '${employeeId}')" 
                                id="filter-all"
                                class="flex-1 px-4 py-2 rounded-lg bg-lime-600 text-gray-900 font-medium">
                            همه (${tasks.length})
                        </button>
    ...
    getemployeeAgentTaskCard(task, employeeId, taskNumber) {
        ...
        return `
            <div class="bg-slate-700 rounded-lg p-4 hover:shadow-xl transition-all relative" data-status="${task.status}">
                ...
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center">
                        <span class="bg-lime-600 text-gray-900 text-xs font-bold rounded-full w-7 h-7 flex items-center justify-center ml-2">${taskNumber}</span>
                ...
                <div class="space-y-2 text-xs text-gray-400 mb-3 pb-3 border-b border-slate-600">
                    <div class="flex items-center">
                        <i class="fas fa-user-shield text-lime-400 ml-1 w-4"></i>
                        <span>عامل: ${task.agentName}</span>
                    </div>
                    <div class="flex items-center">
                        <i class="fas fa-user-graduate text-lime-400 ml-1 w-4"></i>
        ...
        ['all', 'pending', 'in_progress', 'completed'].forEach(s => {
            ...
            if (btn) {
                if (s === status) {
                    ...
                    btn.classList.add('bg-lime-600', 'text-gray-900');
                } else {
                    btn.classList.remove('bg-lime-600', 'text-gray-900');
            ...
            <div id="task-details-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-slate-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                    <div class="p-6 border-b border-slate-700 sticky top-0 bg-slate-800 z-10">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-bold text-white">
                                <i class="fas fa-info-circle text-lime-400 ml-2"></i>
                                جزئیات وظیفه
                            </h3>
                        ...
                        ${task.attachedFile ? `
                            <div class="bg-slate-700 rounded-lg p-4">
                                ...
                                <div class="flex items-center justify-between bg-slate-600 rounded-lg p-3">
                                    <div class="flex items-center space-x-3 space-x-reverse">
                                        <i class="fas fa-file text-lime-400 text-2xl"></i>
                                        <div>
        ...
        const modalHTML = `
            <div id="edit-student-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-slate-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                    <div class="p-6 border-b border-slate-700 sticky top-0 bg-slate-800 z-10">
                        <h3 class="text-lg font-bold text-white">
                            <i class="fas fa-user-edit text-lime-400 ml-2"></i>
                    ...
                    <div class="border-b border-slate-700 bg-slate-800 sticky top-[73px] z-10">
                        <div class="flex space-x-2 space-x-reverse px-6">
                            <button onclick="employeeModule.switchStudentTab('info')" 
                                    id="tab-info"
                                    class="px-4 py-3 font-medium border-b-2 border-lime-500 text-lime-400 transition-all">
    ...
    getChatWithManagerContent(userId) {
        ...
        return `
            <div class="space-y-6">
                ...
                <div class="flex justify-between items-center">
                    <h2 class="text-2xl font-bold text-white">
                        <i class="fas fa-comments text-lime-400 ml-2"></i>
                        گفتگو با مدیر
                    </h2>
                </div>
                    ...
                    <div class="lg:col-span-2">
                        <div class="bg-slate-800 rounded-lg shadow-md">
                            <div class="p-4 border-b border-slate-700">
                                <h3 class="text-lg font-bold text-white">
                                    <i class="fas fa-comment-dots text-lime-400 ml-2"></i>
                                    پیام‌ها
                                </h3>
                            </div>
    ...
    getChatMessage(message, userId) {
        ...
        return `
            <div class="flex ${isMe ? 'justify-end' : 'justify-start'}">
                <div class="max-w-xs lg:max-w-md ${isMe ? 'bg-lime-600' : 'bg-slate-600'} rounded-lg p-3">
            ...
            <div id="create-agent-task-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-slate-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                <i class="fas fa-heading text-lime-400 ml-1"></i>
                                عنوان وظیفه
                            </label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                <i class="fas fa-align-right text-lime-400 ml-1"></i>
                                توضیحات
                            </label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                <i class="fas fa-user-shield text-lime-400 ml-1"></i>
                                تخصیص به عامل
                            </label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">
                                    <i class="fas fa-calendar text-lime-400 ml-1"></i>
                                    تاریخ تحویل
                                </label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">
                                    <i class="fas fa-clock text-lime-400 ml-1"></i>
                                    ساعت تحویل
                                </label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-2">
                                <i class="fas fa-flag text-lime-400 ml-1"></i>
                                اولویت
                            </label>
        ...
        return `
            <div class="flex items-center justify-center h-64">
                <div class="text-center">
                    ...
                    <button onclick="location.reload()" class="mt-4 bg-lime-600 hover:bg-lime-700 text-gray-900 px-4 py-2 rounded-lg">
            ...
            <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-1 flex-shrink-0">
                    ...
                    <button onclick="employeeModule.moveStep('${type}', ${index}, 'up')" 
                            class="text-gray-400 hover:text-lime-300 p-1.5 rounded transition-all ${index === 0 ? 'opacity-30 cursor-not-allowed' : ''}" 
                            title="انتقال به بالا" ${index === 0 ? 'disabled' : ''}>
                        <i class="fas fa-chevron-up text-xs"></i>
                    </button>
                    <!-- Move Down -->
                    <button onclick="employeeModule.moveStep('${type}', ${index}, 'down')" 
                            class="text-gray-400 hover:text-lime-300 p-1.5 rounded transition-all ${index === total - 1 ? 'opacity-30 cursor-not-allowed' : ''}" 
...
EmployeeModule.switchPathTab = function(tab, studentId) {
    ...
    [educationalTab, defenseTab, requirementsTab].forEach(t => {
        if (t) {
            t.classList.remove('border-lime-500', 'text-lime-600');
    ...
    if (tab === 'educational' && educationalTab && educationalContent) {
        ...
        educationalTab.classList.add('border-lime-500', 'text-lime-600');
        ...
        defenseTab.classList.add('border-lime-500', 'text-lime-600');
