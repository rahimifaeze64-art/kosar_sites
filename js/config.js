...
const CONFIG = {
  ...
  UNIVERSITIES: ["دانشگاه قم", "جامعه المصطفی", "سایر (وارد کنید)"],
...
const UTILS = {
  ...
  calculateShares(orderType, totalAmount) {
    ...
    const doctorShare =
      totalAmount * (pricing.doctorShare || pricing.translatorShare || 0);
    ...
    try {
      ...
      setTimeout(() => {
        notification.style.animation =
          "notificationSlide 0.3s ease-out reverse";
        setTimeout(() => {
  ...
  validateForm(data, rules) {
    ...
    for (const field in rules) {
      ...
      if (value && rule.minLength && value.length < rule.minLength) {
        errors[field] =
          `${rule.label} باید حداقل ${rule.minLength} کاراکتر باشد`;
      }

      if (value && rule.maxLength && value.length > rule.maxLength) {
        errors[field] =
          `${rule.label} نباید بیشتر از ${rule.maxLength} کاراکتر باشد`;
      }
